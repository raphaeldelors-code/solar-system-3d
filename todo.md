...
- [x] fix(render): Milky Way narrower + purplish (user photo ref) — plan `plans/036-mw-narrow-purple.md`. Parameterize `bake_milkyway.py` (`--core-sigma/--halo-sigma/--sample-sigma/--purple`); re-bake `--core-sigma 2.8 --halo-sigma 5.5 --sample-sigma 3.8 --purple --star-scale 0.10 --band-scale 0.55`. 1% halo FWHM 12.6→10.2° (19%) / 12.9→9.3° (28%) at gal-long 90°/135°; hue blue-white→violet/magenta. `MILKYWAY_TINT` unchanged (0.3). — `5bce13a`
- [x] fix(intro): skip-intro button appeared on pinned-URL loads with NO intro and could not be dismissed (user repro `?f=earth…&cam=…`); plain reload replayed the dolly. Plan `plans/037-fix-skip-intro.md`. Root cause: `#intro{display:flex}` (id, 1-0-0) outranks the UA `[hidden]` rule (0-1-0) → force-visible wrapper rendered its never-hidden Skip button when `urlPinsView` suppressed the intro. Fix: `#intro[hidden]{display:none}` (the `#palette` pattern) + button `hidden` in the DOM; `finishIntro` sets a `sessionStorage` seen-flag, `introShouldPlay` gains an `alreadySeen` gate (session-scoped: replays in a new tab, not on refresh). Verified live at the exact user URL + fresh/finish/skip/reload cycle. — `580244a`
- [x] fix(render): Ceres (all dwarfs) render planet-sized — `planetRadiusKm`'s 0.8 floor puts Ceres at 71% of Earth's disc (real 7.4%). Plan `plans/038-ceres-dwarf-radius.md`. New `dwarfRadiusKm` tier (0.15 + 0.3·log10) → Ceres 0.377 (23% of Earth), all 5 dwarfs < Mercury; wired through `VisualScale.dwarfRadiusKm` (VISIBLE/TRUE/lerp), `buildScene` kind dispatch, `followDistanceKm(km, dwarf)` (dwarf follow frame d=3.0 vs 6.8); orbit `ANCHORS` unchanged (`solve_scale.py --dwarf` regression gate: 0 fails). Live: Earth-calibrated pixel ratio + vision. — `79d48dc`
- [x] fix(render): selection disc too big — broad annulus (1.55–2.35×) sat _inside_ Saturn's natural ring band (1.24–2.27×) and reached the Moon/Amalthea orbits, so satellites read as sitting on the disc. Plan `plans/039-selection-disc-collar.md`. Replace with a thin hugging collar `RingGeometry(1.03, 1.08)` in the equatorial plane; at the breathing peak (×1.12) the outer edge is 1.2096× — just inside the two binding constraints, Jupiter/Amalthea (floor 1.242×) and Saturn's ring inner edge (1.24×), so it clears every moon orbit on every body. Live before/after vision on Saturn/Jupiter/Earth follow frames: collar hugs the limb, inside Saturn's rings, well inside the Moon/Galilean orbits. — `4fd8890`
- [x] fix(sky): constellation edges hard to trace with figures on — plates capped at a magic 0.85 (only 15% under the line peak 1.0) AND the figure group is added to the scene after the line group, so in three.js's transparent pass the dense plates painted over the thin edge lines wherever they overlap. Plan `plans/040-constellation-figure-dim.md`. Fix: extract `FIGURE_PLATE_MAX_OPACITY` (0.85 → 0.5, a soft underlay at half the line peak) + `renderOrder 1` on the lines/star-dots/emphasis-dots (plates stay 0) so the edges are never buried. Live A/B vision (Ursa Major picked, figures on, labels off): line-over-art traceability 3/10 → 9/10, art visibly dimmer. — `85afea1`
- [x] style(panel): control menu "feels a bit cheap" — full design review + rework. Plan `plans/041-controls-menu-design.md`. CSS-only redesign of the left panel (no DOM/ID changes): group dividers via `:has()` landmarks (Navigate/Display/Actions), a 3-tier button system (🌌 Sky / 🪐 System = accent primary; time controls = secondary; Copy share link / Save screenshot = ghost), uppercase micro-labels + brighter tabular values, panel elevation (box-shadow, 14px radius), inset picked-body info card, 44px/48px touch targets. Gates green (388 tests + tsc + lint + format + build); vision A/B on default / events-expanded / Mars-info states: 5/10 → 7.5/10, no regressions. — `49ff099`
- [x] fix(sky): constellation lines hard to trace where the hairline crosses the bright cream figure fills — plan `plans/042-constellation-line-halo.md`. The 1px `LineBasicMaterial` edge is pale blue; over a large cream plate the contrast is too low and the line "disappears" (user screenshot). A 2D/CSS halo can't work (the label overlay sits ABOVE the canvas), so the fix lives in the same transparent WebGL pass, one renderOrder below the lines. WebGL lines are a fixed 1 device px, so use three's fat-line addon (`Line2`/`LineGeometry`/`LineMaterial`): per constellation three line objects stacked by renderOrder (plate 0 / halo 1 / fat 2 / core 3 / dots 4) — a PURE BLACK `0x000000` halo under-stroke (11 world units), a fat blue line (8 world units, the primary visible stroke), and the kept 1px crisp core. `worldUnits:true` so widths scale with zoom. The halo is pure black on purpose: over cream it cuts a dark groove that carries the contrast, but over the near-black sky black-on-black is INVISIBLE (an initial near-black `#0a0f18` halo A/B-verified to read as a fat grey line on the open sky; pure black removes that). Halo opacity mirrors the blue line's D4 curve (base 0.45 / peak 0.70; picked figures share the emphasis pulse). 4 new regression tests pin the pure-black color, the width/opacity bands, and the 3-object renderOrder stack. Live A/B (headless Chrome, Ursa Major, plates forced full-bright worst case): line-over-cream readability 3/10 → 8/10; native-res crop confirms NO grey-line artifact on the open black sky. — `b53255c`
- [x] F1 fix(ui): cap panel height so a picked body's info card never clips — plan `plans/043-panel-polish-pass.md`. `#panel` was content-box with no max-height, so Mars-picked at 1280×800 measured 828px tall in an 800px viewport (hint + Stellarium credit clipped off-screen). border-box + flex column (h1 pinned, `.panel-body` scrolls, thin scrollbar); cap `max-height: calc(100vh - 44px - 12px - safe-area)`. Verified: Mars bottom 788 ≤ 800, default unchanged. — `cb62008`
- [x] F2 style(ui): disambiguate the two Date labels (Now vs Jump to) — plan 043. Readout row "Date" → "Now", date-picker "Date" → "Jump to"; label column 54→64px so "Jump to" fits one line. — `d655476`
...
- [x] B1 feat(data): live ISS + satellite tracking (SGP4 + CelesTrak TLEs) (plan 044) — `e9790c4`
- [x] B2 feat(data): JPL Horizons live mode (plan 044) — `8b007be`
- [x] B3 feat(data): real starfield from Gaia 1M-star extract (plan 044) — `5064739`
- [x] B4 feat(data): SBDB body-facts panel (plan 044) — `3085842`
...
- [ ] D2 refactor: split main.ts god-file (frameLoop/scrub/searchUi/eventsPanel/contextLoss) (plan 044) — IN PROGRESS: eventsPanel `205b122` + frameLoop `95ce34d` + scrub `c380a6c` + searchUi `cd8e9be` (main.ts 4200→3145); next: contextLoss
