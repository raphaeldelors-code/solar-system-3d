/**
 * Tests for the constellation NAME labels (D7): the elegant lettering sits
 * BESIDE each figure (angular offset along its long axis, not on top of the
 * stars), and fades with the SAME emphasis as its own figure's lines (D4) —
 * the highlight lookup is name-based, so an interleaved child order
 * (lines0, label0, lines1, label1, …) can never fade label k with
 * constellation k+1's emphasis.
 */
...
describe('constellationLabelPose', () => {
  it('offsets the label beyond the figure along its principal axis', () => {
    for (const c of CONSTELLATIONS) {
      const pose = constellationLabelPose(c);
      const dir = constellationCenter(c);
      expect(pose.halfExtent).toBeGreaterThan(0);

      // The label sits at the centroid's direction rotated by a positive
      // margin along the long axis: its angular distance from the center
      // equals the margin (exact spherical offset), so the name clears the
      // figure instead of covering it.
      const margin = Math.max(0.35, pose.halfExtent * 1.15);
      ...
      // placement) — it is offset by the margin from the centroid.
  ...
  it('keeps the label on the same side of the sky as the figure (no wrap-around)', () => {
    for (const c of CONSTELLATIONS) {
      const pose = constellationLabelPose(c);
      const dir = constellationCenter(c);
      const labelDir = pose.labelDir(Math.max(0.35, pose.halfExtent * 1.15));
