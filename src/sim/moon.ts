...
const PERIODIC_LR: ReadonlyArray<readonly [number, number, number, number, number, number]> = [
  [0, 0, 1, 0, 6.28877e6, -2.09054e7],
  [2, 0, -1, 0, 1.27403e6, -3.69911e6],
  [2, 0, 0, 0, 658314, -2.95597e6],
...
export function moonGeocentricEcliptic(tDays: number): MoonGeocentric {
  ...
  const Lprime =
    218.3164477 + (481267.8812342 + (-0.0015786 + (1.0 / 538841.0 - t / 65194000.0) * t) * t) * t;
  // Mean elongation of the Moon
  const D =
    297.8501921 + (445267.1114034 + (-0.0018819 + (1.0 / 545868.0 - t / 113065000.0) * t) * t) * t;
  // Sun's mean anomaly
  const M = 357.5291092 + (35999.0502909 + (-0.0001536 + t / 24490000.0) * t) * t;
  // Moon's mean anomaly
  const Mprime =
    134.9633964 + (477198.8675055 + (0.0087414 + (1.0 / 69699.9 + t / 14712000.0) * t) * t) * t;
  // Argument of latitude
  const F =
    93.272095 + (483202.0175233 + (-0.0036539 + (-1.0 / 3526000.0 + t / 863310000.0) * t) * t) * t;
