...
- [x] D10 Pause-on-visibilitychange + listener audit — 6a4a7ef (frameLoop hidden dep + skip-when-hidden guard; main.ts visibilitychange listener on a page-level AbortController, lastMs reset on return, __debug.hidden) + c8c182c (format fix). E2E: hidden flag tracks the tab. 11/11 E2E pass. Deployed to gh-pages (bundle 5DmW1lwV has visibilitychange); live CDN was still serving the D9 asset at verify time (benign propagation lag — D10 didn't touch index.html).
